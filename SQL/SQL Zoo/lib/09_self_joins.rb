...
def lrt_stops
  ...
  select id, name
  from stops
  join routes on id=stop_id
  where num like '4' and company like 'LRT'
  SQL
end
...
def haymarket_and_leith
  ...
  execute(<<-SQL)
  select distinct routesa.company, routesa.num
  from routes routesa
  join routes routesb on routesa.company = routesb.company
  and routesa.num = routesb.num
  join stops stopsa on stopsa.id = routesa.stop_id
  join stops stopsb on stopsb.id = routesb.stop_id
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
  select distinct stopsb.name, routesa.company, routesa.num
  from routes routesa
  join routes routesb on routesa.company = routesb.company
  and routesa.num = routesb.num
  join stops stopsa on stopsa.id = routesa.stop_id
  join stops stopsb on stopsb.id = routesb.stop_id
  where stopsa.name like 'Craiglockhart'
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  select routesa.num, routesa.company, stopsb.name, routesb.company 
  SQL
end
